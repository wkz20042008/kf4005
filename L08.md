...
1. It is also possible to use `fdisk` with a disk image file, e.g.
     ...
     ```
     {: .code}
To try this out for yourself you'll need to obtain a disk image file. If you're
in PAN S3 or PAN S4, the easiest way to do this is to mount the NAS drive and
...
your machine.
       ...
       ```
       {: .code}
    1. Create a mount point for the NAS drive, e.g. 
...
6. Download a raw disk image, using the command below.
     ...
     ```sh
     $ wget rtlab1/downloads/disk.img
     ```
     {: .code}
...
10. Mounting LVM volumes from a disk image is trickier and outside the scope of this module. However, if you're interested, there's a 
